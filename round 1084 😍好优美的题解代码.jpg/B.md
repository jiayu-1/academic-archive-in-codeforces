...
正确的推导过程为：
仔细读题，发下上述自认为正确的推导，实则忘记题意。题意是“每次任意删除一个”
既然“每次任意删除一个”，所以想到“保留谁”,会使得之后能达到最佳结果
尽量剩余少 --> 每次必含有递减块 -->发现递减块是紧挨着的两个元素 ai >ai+1 --> 
每次只保留 ai >ai+1，其余依次删除其余，然后 ai >ai+1二选一删除
所以，如果序列里有一对 ai >ai+1，则可以依次删除直至1
如果无，则说明原序列是非递减，所以不用删除直接是n
